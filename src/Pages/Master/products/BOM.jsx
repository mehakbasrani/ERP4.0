...
const totalRecords = await getPageInfo(axios, MasterUrl.getPageInfo, "prodbom");
...
export default function Operation() {
  ...
  const addNew = async () => {
    const id2 = await putNewId(axios, MasterUrl.putNewId, "prodbom");
    console.log(`id2:${id2}`);
  ...
  const handleExportData = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ProductBOMDataSheet");
            ...
            <Table.Body>
              {pageData?.map((contact) => (
                ...
                >
                  ...
                  <Table.Cell>{contact.itemname}</Table.Cell>
                  <Table.Cell>{contact.unit}</Table.Cell>
                  <Table.Cell>{contact.cons}</Table.Cell>
                  <Table.Cell>{contact.rate}</Table.Cell>
                  <Table.Cell>{contact.amount}</Table.Cell>
                </Table.Row>
              ))}
            </Table.Body>
          </Table>
