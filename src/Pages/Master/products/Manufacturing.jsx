...
const header = [" ", "Style ID", "Style Name", "Process", "Cost"];

//get total no of pages from items table
const totalRecords = await getPageInfo(axios, MasterUrl.getPageInfo, "prodmfg");
...
export default function Manufacturing() {
  ...
  const addNew = async () => {
    const id2 = await putNewId(axios, MasterUrl.putNewId, "prodmfg");
    console.log(`id2:${id2}`);
  ...
  const handleExportData = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ProductManufacturingDataSheet");
            ...
            <Table.Body>
              {pageData?.map((contact) => (
                ...
                >
                  ...
                  <Table.Cell>{contact.process}</Table.Cell>
                  <Table.Cell>{contact.cost}</Table.Cell>
                </Table.Row>
              ))}
            </Table.Body>
          </Table>
