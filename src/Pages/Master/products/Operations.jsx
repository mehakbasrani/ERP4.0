...
const totalRecords = await getPageInfo(axios, MasterUrl.getPageInfo, "prodop");
...
export default function Operation() {
  ...
  const addNew = async () => {
    const id2 = await putNewId(axios, MasterUrl.putNewId, "prodop");
    console.log(`id2:${id2}`);
  ...
  const handleExportData = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ProductOperationDataSheet");
              ...
              {pageData &&
                pageData.map((contact) => (
                  ...
                  >
                    ...
                    <Table.Cell>{contact.opname}</Table.Cell>
                    <Table.Cell>{contact.opshname}</Table.Cell>
                    <Table.Cell>{contact.machine}</Table.Cell>
